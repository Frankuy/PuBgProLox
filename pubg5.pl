...
attack :- playerHealth(X), npcEquipment(Id,_,Y), playerEquip(_,DamageEquip), Z is X-Y, retract(playerHealth(X)), asserta(playerHealth(Z)), npcHealth(Id,HpNPC), HpNPCNow is HpNPC-DamageEquip, deleteEnemy(Id, HpNPCNow).

deleteEnemy(Id, Hp) :- Hp=<0, retract(npcEquipment(Id,NamaSenjata,_)), retract(npcPosition(Id,X,Y)), retract(npcHealth(Id,_)),asserta(weaponPosition(NamaSenjata,X,Y)),!.
deleteEnemy(Id, Hp) :- retract(npcHealth(Id,_)), asserta(npcHealth(Id,Hp)).
